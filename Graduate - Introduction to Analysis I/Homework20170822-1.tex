...
When $x \ne \frac{q}{p^n}$, assume there exists two sequences for representing $x$, denoted by $\{a_i\}$ and $\{b_i\}$. According to this assumption, there must exist a $j$, s.t. $a_j \ne 9$. Let $k$ be the first position of difference between these two sequences, then $|\sum{a_i}-\sum{b_i}| >= \frac{1}{p^k}-\sum_{i=k+1}^{\infty}\frac{|a_i-b_i|}{p^i} > \frac{1}{p^j}$. So this contradicts with our assumption, which means $x$ only has one sequence in this form.
\end{proof}
\bigskip
...
\textbf{Solution:}
\begin{proof}
Let $E\subset F$ is an upper bounded set, and $T$ is the set of upper bounds of $E$. 
\end{proof}
\bigskip
