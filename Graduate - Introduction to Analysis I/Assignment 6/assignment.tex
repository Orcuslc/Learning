...
$$
For the continuity of $h$, we need to show that $\forall y_0 \in [0, 1]$, $\forall \epsilon > 0$, $\exists \delta > 0$, when $|y-y_0| <\delta $, we have $h(y)-h(y_0) = |\int_{0}^{1}f(x, y)dx-\int_{0}^{1}f(x, y_0)dx| < \epsilon$. Since $f(x, y)$ is continuous in $y$ for each $x$ in a closed set, then $f(x, y)$ is uniformly bounded, then for each $x$, $\exists \delta_1$, when $|y-y_0| <\delta_1 $, $|f(x, y)-f(x, y_0)| < \epsilon$. Then 
$$
...
\solution{(i)}{
$$
\lim_{n\to\infty}\int_{1}^{n} fdx = \lim_{n\to\infty}\sum_{m = 1}^{n}\frac{(-1)^m}{m} = -\ln2,
$$
But
$$
