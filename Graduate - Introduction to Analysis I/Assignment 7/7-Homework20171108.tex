...
% Let $\mathcal{F} = \bigcup O_k $, where $O_k = \bigcup I_{k_j} $ is an open set covering $E$. Then since $E$ is a set of outer measure 0, for each $\epsilon > 0$, there exists $O_k $, s.t. $O_k \supset E $ and $m(O_k) < \epsilon$. Then for each $x\in E$, $x\in O_k \in \mathcal{F}$. This means $\mathcal{F}$ is a (general) Vitali cover of $E$. By Problem 6.6 (Gereral case of Vitali covering lemma), for $\epsilon_n = \frac{\epsilon}{2^n}$, there exists 
