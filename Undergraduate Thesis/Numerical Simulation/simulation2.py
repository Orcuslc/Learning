...
def kernel(t):
	...
	for i in range(1, t.size):
		dt = t[i] - t[i-1]
		stp = np.sqrt(dt)
		du_re = -gamma[i-1]*u_re[i-1]-omega*u_im[i-1]+b_re[i-1]+scipy.real(f(t[i-1]))+sigma*randn()/stp
		du_im = omega*u_re[i-1]-gamma[i-1]*u_im[i-1]+b_im[i-1]+scipy.imag(f(t[i-1]))+sigma*randn()/stp
