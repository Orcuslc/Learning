...
\subsection{均值}
当$t$固定时, 由(\ref{Sb})式可知$b(t)$的通解第一项为常数, 第二项中只有$b_0$是一个随机变量. 那么由(\ref{init_values})可知, $b(t)$通解第二项的期望为$(\E[b_0]-\hat{b})e^{\lambda_b(t-t_0)}$. 而由于
...
在上式中$R$为E-M算法的步长, 且
$$
\mathrm{d}W = \sqrt{\Delta t}*\tt{randn()}.
$$
