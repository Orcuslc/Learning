...
\begin{abstract}
本文研究了一个Kalman滤波模型中重要的随机微分方程组的解, 利用特征函数和It\^o公式补全了解的一阶和二阶统计量的计算过程, 并使用Monte Carlo模拟对理论结果进行了数值检验. \\
\noindent{\heiti 关键字:} 随机微分方程组; 统计量; 数值模拟
\end{abstract}
...
&\quad+ 2E\left[\left(\int_s^t(\gamma_0-\hat{\gamma})e^{-d_\gamma(s'-t_0)}ds'\right)\left(\sigma_\gamma\int_s^t\int_{t_0}^{s'}e^{-d_\gamma(s'-x)}dW_\gamma(x)ds'\right)\right] \\
&\quad+ \sigma_\gamma^2E\left[\left(\int_s^t\int_{t_0}^{s}e^{-d_\gamma(s'-s)}dW_\gamma(s)ds'\right)^2\right] \\
&= \frac{1}{d_\gamma^2}\left(e^{-d_\gamma(s-t_0)}-e^{-d_\gamma(t-t_0)}\right)^2(E[\gamma_0^2]-E[\gamma_0]^2) + \sigma_\gamma^2E\left[\left(\int_{t_0}^t \frac{1}{d_\gamma}\left(1-e^{-d_\gamma}(t-x)\right)dW_\gamma(x)\right)^2\right]
\end{split}
\]
...
由命题\ref{multiVariable gaussian exp 3}可知, 在上式中, 被积的最后一项为
...
\left\{
\begin{aligned}
&Re(u_0), Im(u_0) \sim \mathcal{N}(0, 1/2), \quad i.i.d. \\
&Re(b_0), Im(b_0) \sim \mathcal{N}(0, 1/2), \quad i.i.d. \\
...
在上式中$R$为E-M算法的步长, 且
$$
dW = \sqrt{\Delta t}*randn().
$$
由于模拟$u(t)$和$b(t)$时, 我们需要对其实部和虚部分别进行模拟. 为保证三个微分方程中白噪声的强度一致, 需要令
$$W_{Re(u)}(t) = \frac{1}{\sqrt{2}}W(t),$$
其余项和初值也做相同的处理. \\

对$R=1$进行100,000次Monte Carlo模拟\cite{robert2004monte}, $u(t)$, $b(t)$的实部和虚部,$\gamma(t)$的期望, $u(t)$, $b(t)$和$\gamma(t)$的方差的模拟结果如下图\cite{mckinney2012python}\cite{hunter2007matplotlib}, 其中蓝色的曲线为模拟产生的值, 绿色的曲线是理论计算得到的值. \\
% \begin{figure}[!ht]
% \centering
...
\centering
\subfloat[E(Re(u))]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/EuRe.png}
  }\hfill
\subfloat[E(Im(u))]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/EuIm.png}
  }\\
\subfloat[E(Re(b))]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/EbRe.png}
  }\hfill
\subfloat[E(Im(b))]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/EbIm.png}
  }\\
\subfloat[E($\gamma$)]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/Egamma.png}
  }\hfill
\subfloat[Var(u)]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/Varu.png}
  }\\
\subfloat[Var(b)]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/Varb.png}
  }\hfill
\subfloat[Var($\gamma$)]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/Vargamma.png}
  }
...
\centering
\subfloat[$Im(Cov(u, u^*))$]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/CovuustarRe.png}
}\hfill
\subfloat[$Im(Cov(u, u^*))$]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/CovuustarIm.png}
}\\
\subfloat[$Re(Cov(u, \gamma))$]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/CovugammaRe.png}
}\hfill
\subfloat[$Im(Cov(u, \gamma))$]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/CovugammaIm.png}
}\\
\subfloat[$Re(Cov(u, b)$]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/CovubRe.png}
}\hfill
\subfloat[$Im(Cov(u, b))$]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/CovubIm.png}
}\\
\subfloat[$Re(Cov(u, b^*))$]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/CovubstarRe.png}
}\hfill
\subfloat[$Im(Cov(u, b^*))$]{
  \includegraphics[width = .4\textwidth]{NumericalSimulation/res/CovubstarIm.png}
}
