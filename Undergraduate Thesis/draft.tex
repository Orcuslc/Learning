...
\begin{split}
u(t) &= e^{-J(t_0, t)+\hat{\lambda}(t-t_0)}u_0 + \int_{t_0}^t (b(s)+f(s))e{-J(s, t)+\hat{\lambda}(t-s)}ds \\
& + \sigma\int_{t_0}^{t}e{-J(s, t)+\hat{\lambda}(t-s)}dW(s).
\end{split}
\end{equation}
...
那么$\textbf{x}$的概率密度为
$$f_x(x_1, \cdots, x_k) = \frac{exp(-\frac{1}{2}(\textbf{x}-\mu)^\top)\Sigma^{-1}(\textbf{x}-\mu)}{\sqrt{(2\pi)^k|\Sigma|}}$$
\end{proposition}
...
$$\phi(t) = E[exp(i\textbf{t}\textbf{X}^\top)], \quad \textbf{t} = (t_1, t_2, \cdots, t_n)\in \mathbb{R}^n.$$
\end{definition}
以下为高斯分布的特征函数.
...
$$E[\sigma_b\int_{t_0}^{t} e^{\lambda_b(t-s)dW_b(s)}] = 0.$$
从而
...
\begin{split}
var(b(t)) &= var((b_0 - \hat{b})e^{\lambda_b(t-t_0)}) + var(\sigma_b\int_{t_0}^{t}e^{\lambda_b(t-s)}dW_b(s)) \\
&= (e^{\lambda_b(t-t_0)})^2var(b_0) + var(\sigma_b\int_{t_0}^t e^{\lambda_b(t-s)}dW_b(s)) \\
&= Re\{e^{2(-\gamma_b+i\omega_b)(t-t_0)}var(b_0)\} + E[(\sigma_b\int_{t_0}^t e^{\lambda_b(t-s)}dW_b(s))^2] \\
&= e^{-2\gamma_b(t-t_0)}var(b_0) + Re\{\sigma_b^2E[\int_{t_0}^t e^{2\lambda_b (t-s)}dt]\}
\end{split}
\end{equation}
...
\begin{split}
cov(b(t), b(t)^*) &= E[(b(t) - E[b(t)])(b(t)^* - E[b(t)^*])] \\
&= E[(b_0 - E[b_0]e^{\lambda_b (t-t_0)}+\sigma_b\int_{t_0}^{t} e^{\lambda_b (t-s)}dW_b(s))\cdot \\
&\quad ((b_0^* - E[b_0^*])e^{\lambda_b(t-t_0)}+\sigma_b\int_{t_0}^t e^{\lambda_b (t-s)}dW_b(s))] \quad\cdots(\ref{b_stat_1}) \\
&= E[(b_0 - E[b_0])(b_0^* - E[b_0^*])e^{2\lambda_b (t-t_0)}] + \sigma_bE[(b_0 - E[b_0])\int_{t_0}^{t}e^{\lambda_b(t-s)}dW_b(s)]\\
&+ \sigma_bE[(b_0^*-E[b_0^*])e^{\lambda_b(t-t_0)}\int_{t_0}^{t}e^{\lambda_b(t-s)}dW_b(s)] + \sigma_b^2E[(\int_{t_0}^{t}e^{\lambda_b(t-s)}dW_b(s))^2]
\end{split}
\end{equation}
在上式的第二项中, 由于$b_0$与$\int_{t_0}^{t} e^{\lambda_b (t-s)}dW_b(s)$相互独立, 故由It\^{o}积分的性质(\ref{Itoint_property})可知
\begin{equation}
\sigma_bE[(b_0 - E[b_0])\int_{t_0}^{t}e^{\lambda_b(t-s)}dW_b(s)] = \sigma_bE[b_0 - E[b_0]]E[\int_{t_0}^{t}e^{\lambda_b(t-s)}dW_b(s)] = 0
\end{equation}
...
&= E[(b_0 - E[b_0]e^{\lambda_b (t-t_0)}+\sigma_b\int_{t_0}^{t} e^{\lambda_b (t-s)}dW_b(s)) \cdot \\
&\quad ((\gamma_0-E[\gamma_0])e^{-d_\gamma(t-t_0)}+\sigma_\gamma \int_{t_0}^{t} e^{-d_\gamma(t-s)}dW_\gamma(s))] \\
&= E[(b_0-E[b_0])(\gamma_0-E[\gamma_0])]e^{(\lambda_b-d_\gamma)(t-t_0)} \\
...
\begin{split}
E[u(t)] &= e^{\hat{\lambda}(t-t_0)}E[e^{-J_0(t_0, t)}u_0] + \int_{t_0}^t e^{\hat{\lambda}(t-s)}E[b(s)e^{-J(s, t)}]ds  \\
&+ \sigma\int_{t_0}^{t} e^{\hat{\lambda}(t-s)}f(s)E[e^{-J(s, t)}]ds.
\end{split}
\end{equation}
...
$$E[ze^{ibx}] = (E[z]+ibcov(z, x))e^{ibE[x]-\frac{1}{2}b^2var(x)}$$
...
\frac{\partial \phi_\textbf{v}(\textbf{s})}{\partial s_2} = \frac{1}{(2\pi)^3}\int iy_0e^{i\textbf{s}^\top\textbf{v}}g(\textbf{v})d\textbf{v} = iE[y_0e^{i\textbf{s}^\top\textbf{v}}].
\end{equation}
...
E[y_0e^{ibx_0}] = (E[y_0]+icov(x_0, y_0)b)exp(ibE[x_0] - \frac{1}{2}var(x_0)b^2)
\end{equation}
\begin{equation} \label{partial over s3 at s}
E[w_0e^{ibx_0}] = (E[w_0]+icov(x_0, w_0)b)exp(ibE[x_0] - \frac{1}{2}var(x_0)b^2)
\end{equation}
结合(\ref{partial over s2 at s}), (\ref{partial over s3 at s})即有
$$E[ze^{ibx}] = (E[z]+ibcov(z, x))e^{ibE[x]-\frac{1}{2}b^2var(x)}.$$
\end{proof}
由此我们立刻可以得到
\begin{corollary} 在(\ref{multivariable gaussian exp 1})的条件下,
$$E[ze^{bx}] = (E[z]+bcov(z, x))e^{bE[x]+\frac{1}{2}b^2var(x)}.$$
\end{corollary}
