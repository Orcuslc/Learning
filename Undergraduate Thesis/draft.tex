...
\begin{abstract}
本文研究了随机参数化Kalman滤波模型中一个重要随机微分方程组的解, 利用It\^o积分和特征函数补全了解的一阶和二阶统计量的计算过程, 并对理论结果进行了数值检验. \\
...
\section{导言}
\subsection{问题简介}
滤波是从自然信号中得到最佳统计估计的过程. 在实际应用中, 研究者需要对自然界中的物理过程进行参数化表示, 而在此过程中, 不合适的采样率和不完全的物理认知会造成模型的系统误差, 最终使得对混乱信号的滤波结果受到较大的影响. 为此, B. Gershgorin等提出了随机参数化Kalman滤波模型\cite{gershgorin2010improving}\cite{gershgorin2010test}, 以降低模型的系统误差, 改进滤波结果.

自然界中的信号可近似的用Langevin方程的解进行模拟\cite{langevin1908theorie}:
\[
\frac{\mathrm{d}u(t)}{\mathrm{d}t} = -\gamma(t)u(t) + \ii\omega u(t)+\sigma W(t)+f(t),
\]
...
\left\{
\begin{split}
& \frac{du(t)}{dt} = (-\gamma(t)+\ii\omega)u(t)+b(t)+f(t)+\sigma W(t), \\[5pt]
% \end{equation}
% \begin{equation}
& \frac{db(t)}{dt} = (-\gamma_b+\ii\omega_b)(b(t)-\hat{b})+\sigma_b W_b(t), \\[5pt]
% \end{equation}
% \begin{equation}
...
在(\ref{E0})中, $\omega$为$u(t)$的振荡频率, $f(t)$为外部驱动力, $\sigma$表示白噪声$W(t)$的强度. 此外, 参数$\gamma_b$和$d_\gamma$表示振荡阻尼, $\sigma_b$和$\sigma_\gamma$分别表示加性和乘性修正((\ref{E0})中第2, 3式)中白噪声的强度. $\hat{b}$和$\hat{\gamma}$分别表示$b(t)$和$\gamma(t)$的固定平均偏差修正, $\omega_b$表示加性噪声的频率. 白噪声$W_\gamma(t)$是实值函数, 而$W(t)$及$W_b(t)$均为复值, 且其实部和虚部均为独立的白噪声. 
...
\[
\begin{split}
&\E\left[y_0e^{ibx_0}\right] = (\E[y_0]+\ii\Cov(x_0, y_0)b)\exp(\ii b\E[x_0] - \frac{1}{2}\Var(x_0)b^2) \\[5pt]
&\E\left[w_0e^{ibx_0}\right] = (\E[w_0]+\ii\Cov(x_0, w_0)b)\exp(\ii b\E[x_0] - \frac{1}{2}\Var(x_0)b^2)
\end{split}
\]
...
\quad 对$R=1$进行100,000次模拟\cite{robert2004monte}, $u(t)$, $b(t)$的实部和虚部,$\gamma(t)$的期望, $u(t)$, $b(t)$和$\gamma(t)$的方差, 以及各项协方差的模拟结果如下图\cite{mckinney2012python}\cite{hunter2007matplotlib}. \\
% \begin{figure}[!ht]
% \centering
...
\caption{Simulation of Expectations and Variances, $n = 100,000$}
...
\caption{Simulation of Covariances, $n = 100,000$}
\label{Simulation of Covariances}
\end{figure}

下面我们测试了模拟次数为$n = 1,000,000$时的期望, 方差和协方差的模拟结果:
...
\caption{Simulation of Expectations and Variances, $n = 1,000,000$}
\label{Simulation of Expectations and Variances}
\end{figure}
...
\caption{Simulation of Covariances, $n = 1,000,000$}
\label{Simulation of Covariances}
\end{figure}
...
\subsection{讨论和展望}
从数值模拟结果我们可以看出, 当模拟次数$n = 100,000$时, 对期望的模拟结果和理论值基本重合, 方差的模拟结果和理论值有着$O(10^{-2})\sim O(10^{-3})$的误差, 而协方差的误差在$O(10^{-3})\sim O(10^{-4})$的数量级. 而当模拟次数$n = 1,000,000$时, 期望和方差的模拟结果和理论值几乎完全一致, $\Cov(u, u^*)$及$\Cov(u, \gamma)$的误差为$O(10^{-2})$, $\Cov(u, b)$和$\Cov(u, b^*)$的误差为$O(10^{-4})$. 由模拟结果可以知道, $\Cov(u, b)$和$\Cov(u, b^*)$的误差随着模拟次数的增加而线性降低, 故我们有足够的理由认为当$n \to \infty$时其模拟结果和理论解会趋于一致. 但是$\Cov(u, \gamma)$和$\Cov(u, u^*)$的模拟误差没有随着模拟次数的增加而降低, 其原因尚未确定.
