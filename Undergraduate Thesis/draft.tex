...
&= Re\left\{e^{2(-\gamma_b+i\omega_b)(t-t_0)}Var(b_0)\right\} + E\left[(\sigma_b\int_{t_0}^t e^{\lambda_b(t-s)}dW_b(s))^2\right] \\
...
于是由It\^{o}积分的性质有
\begin{equation}
E\left[|u(t)|^2\right] = E\left[|A|^2\right]+ E\left[|B|^2\right] + E\left[|C|^2\right] + 2Re{(A*B)}.
\end{equation}
...
利用命题\ref{multiVariable gaussian exp 3}可知, 在上式中, 被积的最后一项为
\begin{equation}
\begin{split}
E&\left[(b(s)+f(s))e^{-J(s, t)+\hat{\lambda}(t-s)}\left[(b(r)+f(r))e^{-J(r, t)+\hat{\lambda}(t-r)}\right]^*\right] \\
&= e^{-\hat{\gamma}(2t-s-r)+i\omega(r-s)-E[J(s, t)]-E[J(r, t)]+\frac{1}{2}Var(J(s,t))+\frac{1}{2}Var(J(r, t))+Cov(J(s, t), J(r, t))}\cdot \\
& 
\end{split}
\end{equation}
