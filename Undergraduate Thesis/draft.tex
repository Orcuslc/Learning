...
我们不妨考虑当阻尼系数$\gamma(t)$为常数$\bar{d}$的情形, 此时(\ref{E0})成为
\begin{equation}
\begin{split}
&\frac{du(t)}{dt} = (-\bar{d}+\omega i)u(t)+b(t)+f(t)+\sigma \frac{dW_u(t)}{dt}, \\
&\frac{db(t)}{dt} = (-\gamma_b+\omega_bi)(b(t)-\hat{b})+\sigma_b\frac{dW_b(t)}{dt}
\end{split}
\end{equation}
我们令参数为
\begin{equation}
\left\{
\begin{aligned}
&\bar{d} = 1.5, \sigma = 1, \omega = 2 \\ 
&\gamma_b = 0.1\bar{d} = 0.15, \sigma_\gamma = 5\sigma = 5 \\
&\sigma_b = 5\sigma = 5, \omega_b = \omega = 2
\end{aligned}
\end{equation}
